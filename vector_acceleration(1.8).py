...
for frame in range(0,600):#600 = 10 segundos de 60 frames
    if (coordenadas[0] > width) or (coordenadas[0] < 0)  :#se x passa o valor de largura ou x é menor do que 0
                              # (serve para o x ultrapassar a esquerda)
        color = np.random.randint(0, high=256, size=(3,)).tolist()
        velocidade[0] = velocidade[0] * -1


    if (coordenadas[1] > height) or (coordenadas[1] < 0):#se y passa o valor de altura
                               #(serve para o y ultrapassar acima)
        color = np.random.randint(0, high=256, size=(3,)).tolist()
        velocidade[1] = velocidade[1] * 1.05

        coordenadas[1] = (coordenadas[1] + velocidade[1]) % height
